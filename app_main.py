# file: app_main.py (V2.4 - 在泵输入框旁显示实时值)
...
class PumpActionDialog(QDialog):
    ...
    def get_selected_params(self):
        ...
        if 'kamoer' in pump_id: params['speed'] = float(self.speed_input.text())
...
class SubsystemWidget(QWidget):
    ...
    def _init_ui(self):
        ...
        for pump_conf in self.config['pumps']:
            pump_id = pump_conf['id']; is_peristaltic = 'kamoer' in pump_id
            ...
            self.pump_widgets[pump_id] = {
                ...
                'value_label': QLabel("当前值: 0.00"), # 新增：用于显示实时值的标签
                'direction_box': QComboBox()
            }
            self.pump_widgets[pump_id]['direction_box'].addItems(["正转", "反转"])
            
            # 第1行: 标签和方向
            pumps_layout.addWidget(QLabel(f"<b>{pump_conf['description']}</b>"), row, 0, 1, 2)
            pumps_layout.addWidget(QLabel("方向:"), row, 2)
            pumps_layout.addWidget(self.pump_widgets[pump_id]['direction_box'], row, 3)
            
            # 第2行: 参数输入、实时值和按钮
            param_label = "转速(RPM):" if is_peristaltic else "流量(ml/min):"
            pumps_layout.addWidget(QLabel(param_label), row + 1, 0)
            pumps_layout.addWidget(self.pump_widgets[pump_id]['input'], row + 1, 1)
            pumps_layout.addWidget(self.pump_widgets[pump_id]['value_label'], row + 1, 2) # 添加实时值标签
            pumps_layout.addWidget(self.pump_widgets[pump_id]['start_btn'], row + 1, 3)
            pumps_layout.addWidget(self.pump_widgets[pump_id]['stop_btn'], row + 1, 4)
            
            # 第3行: 状态
            pumps_layout.addWidget(self.pump_widgets[pump_id]['status_label'], row + 2, 0, 1, 5)
            row += 3
        
        pumps_group.setLayout(pumps_layout); top_layout.addWidget(pumps_group); chart_group = QGroupBox("实时数据"); chart_layout = QVBoxLayout(); self.plot_widget = pg.PlotWidget(); self.plot_widget.setBackground('w'); self.plot_widget.showGrid(x=True, y=True); self.plot_widget.addLegend(); self.export_button = QPushButton("导出Excel数据"); chart_layout.addWidget(self.plot_widget); chart_layout.addWidget(self.export_button); chart_group.setLayout(chart_layout); top_layout.addWidget(chart_group, 1); splitter.addWidget(top_widget); self.protocol_widget = ProtocolWidget(self.config, self.parent_window); splitter.addWidget(self.protocol_widget); splitter.setSizes([500, 300]); main_layout.addWidget(splitter)
    def connect_signals(self):
...
class ControlSystemWindow(QMainWindow):
    ...
    def update_ui(self):
        try:
            ...
            if status_data:
                ...
                devices_status = status_data.get('devices', {}); self._update_power_status(devices_status); self._update_subsystem_status(self.subsystem_A_widget, self.config['subsystem_A'], devices_status); self._update_subsystem_status(self.subsystem_B_widget, self.config['subsystem_B'], devices_status)
        except Empty: pass
    ...
    # ★★★ 核心修改 2: 更新子系统状态的逻辑，增加对新 value_label 的更新 ★★★
    def _update_subsystem_status(self, subsystem_widget, subsystem_config, devices_status):
        for pump_conf in subsystem_config['pumps']:
            ...
            if pump_status:
                ...
                # 更新状态栏
                subsystem_widget.pump_widgets[pump_id]['status_label'].setText(f"状态: {'运行中' if is_running else '已停止'}")
                
                # 更新实时值标签
                is_peristaltic = 'kamoer' in pump_id
                current_value = speed if is_peristaltic else flow
                subsystem_widget.pump_widgets[pump_id]['value_label'].setText(f"当前: {current_value:.2f}")
...
class ResourceManager:
    """管理所有系统中的设备，确保任何设备不会被重复控制。"""
    def __init__(self, system_sets):
        self.all_devices = []
        for system_set in system_sets:
            self.all_devices.append(system_set['power_supply'])
            self.all_devices.extend(system_set['subsystem_A']['pumps'])
            self.all_devices.extend(system_set['subsystem_B']['pumps'])
        self.locked_devices = set()
    def get_available_devices_by_type(self, device_type):
        available = []
        for device in self.all_devices:
            if device['type'] == device_type and device['id'] not in self.locked_devices:
...
class DebugWindow(QMainWindow):
    ...
    def __init__(self, device_config):
        super().__init__(); self.config = device_config; self.setWindowTitle(f"调试: {self.config['description']}"); self.resize(600, 200); self.widgets = {}; self.command_queue, self.status_queue, self.log_queue, self.process = None, None, None, None; self._init_ui(); self._connect_signals(); self._start_backend()
    def _init_ui(self):
        central_widget = QWidget(); self.setCentralWidget(central_widget); layout = QGridLayout(central_widget); dev_type = self.config['type']
        if dev_type == 'gpd_4303s': self._create_power_debug_ui(layout)
        elif dev_type == 'kamoer': self._create_pump_debug_ui(layout, is_peristaltic=True)
        elif dev_type == 'oushisheng': self._create_pump_debug_ui(layout, is_peristaltic=False)
        self.status_label = QLabel("状态: 未知"); layout.addWidget(self.status_label, 3, 0, 1, 5)
    ...
    def update_ui(self):
        try:
            status = self.status_queue.get_nowait().get('devices', {}).get(self.config['id'], {})
            if not status: return
            dev_type = self.config['type']
            if dev_type == 'gpd_4303s': on = status.get('output_on', False); v1 = status.get('ch1_voltage', 0); c1 = status.get('ch1_current', 0); v2 = status.get('ch2_voltage', 0); c2 = status.get('ch2_current', 0); self.status_label.setText(f"CH1: {v1:.3f}V/{c1:.3f}A | CH2: {v2:.3f}V/{c2:.3f}A | 输出: {'开' if on else '关'}"); self.widgets['output'].setChecked(on)
            else: run = status.get('is_running', False); s = status.get('speed_rpm', 0); f = status.get('flow_rate_ml_min', 0); self.status_label.setText(f"状态: {'运行中' if run else '停止'} | 转速: {s:.2f} | 流量: {f:.2f}")
        except Empty: pass
        except Exception as e: print(f"Debug window UI update error: {e}")
    def on_set_power(self, channel):
        try: v_in, c_in = (self.widgets['ch1_v'], self.widgets['ch1_c']) if channel == 1 else (self.widgets['ch2_v'], self.widgets['ch2_c']); params = {'device_id': self.config['id'], 'channel': channel, 'voltage': float(v_in.text()), 'current': float(c_in.text())}; self.command_queue.put({'type': 'set_power_voltage', 'params': params}); self.command_queue.put({'type': 'set_power_current', 'params': params})
    ...
    def on_start_pump(self):
        try:
            ...
            if self.config['type'] == 'kamoer': params['speed'] = float(self.widgets['input'].text())
