# file: app_main.py (V2.8 - 添加全局电源控制UI)
...
class ControlSystemWindow(QMainWindow):
    ...
    def _create_shared_controls(self):
        ...
        layout.addWidget(QLabel("数据采样间隔(秒):"))
        layout.addWidget(self.shared_widgets['log_interval_input'])
        layout.addStretch()
        layout.addWidget(self.shared_widgets['open_main_power_btn'])
        layout.addWidget(self.shared_widgets['close_main_power_btn'])
        layout.addWidget(self.shared_widgets['emergency_stop_btn'])
        group.setLayout(layout)
        return group
    ...
    # ★★★ 核心修改 3: 新增全局电源控制的槽函数 ★★★
    def on_open_main_power(self):
        self.command_queue.put({'type': 'open_main_power'})

    def on_close_main_power(self):
        self.command_queue.put({'type': 'close_main_power'})
        
    def on_set_power_channel(self, channel):
        subsystem_widget = self.subsystem_A_widget if channel == self.config['subsystem_A']['channel'] else self.subsystem_B_widget
        widgets = subsystem_widget.power_ch_widgets
        try:
    ...
    def on_toggle_channel_output(self, channel, checked):
        ...
        try:
            ...
            widgets['output_btn'].setChecked(not checked)
    ...
    def on_save_chart(self, subsystem_letter):
        ...
        if filename:
            try:
                exporter = ImageExporter(subsystem_widget.plot_widget.getPlotItem()); exporter.export(filename)
